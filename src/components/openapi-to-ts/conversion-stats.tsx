...
import type { OpenApiToTsResult } from '@/lib/types';

interface StatsProps {
  result: OpenApiToTsResult;
  fromCache: boolean;
}
...
export function OpenApiToTsConversionStats({ result, fromCache }: StatsProps) {
  return (
    <div className="flex flex-wrap items-center gap-2 py-2">
      ...
      >
        <StatBadge label="Schemas" value={result.stats.schemasConverted} />
      </motion.div>
      <motion.div
        initial={{ opacity: 0, y: 4 }}
        animate={{ opacity: 1, y: 0 }}
      ...
      >
        <StatBadge label="Endpoints" value={result.stats.endpointsTyped} />
      </motion.div>
      <motion.div
        initial={{ opacity: 0, y: 4 }}
        animate={{ opacity: 1, y: 0 }}
      ...
      >
        <StatBadge label="Enums" value={result.stats.enumsGenerated} />
      </motion.div>
      <motion.div
        initial={{ opacity: 0, y: 4 }}
        animate={{ opacity: 1, y: 0 }}
      ...
      >
        <StatBadge label="Refs" value={result.stats.refsResolved} />
      </motion.div>
      <motion.div
        initial={{ opacity: 0, y: 4 }}
        animate={{ opacity: 1, y: 0 }}

...
import type { TailwindConversionResult, TailwindConversionOptions } from '@/lib/tailwind-types';
import type { AIProvider } from '@/lib/types';
import { CONVERT_COOLDOWN_MS, MAX_CODE_SIZE } from '@/lib/constants';
import type { RateLimitInfo } from './use-conversion';

interface UseTailwindConversionResult {
  result: TailwindConversionResult | null;
  isConverting: boolean;
  error: string | null;
  fromCache: boolean;
  rateLimitInfo: RateLimitInfo;
  isRateLimited: boolean;
  convert: (input: string, options: TailwindConversionOptions, provider?: AIProvider) => Promise<void>;
}
...
export function useTailwindConversion(): UseTailwindConversionResult {
  const [result, setResult] = useState<TailwindConversionResult | null>(null);
  ...
  const convert = useCallback(async (input: string, options: TailwindConversionOptions, provider?: AIProvider) => {
    if (cooldownRef.current || isRateLimited) return;
    if (!input.trim()) {
      setError('Please enter some Tailwind classes to convert.');
      return;
    }
    ...
    try {
      ...
      const data: TailwindConversionResult = await response.json();
      setResult(data);
    } catch {
